...
Based on scanning all 21 editions (2000-2020), there are 78 distinct metrics tracked. These are categorized by uniqueness to CIA Factbook vs overlap with other data sources.

---

### UNIQUE TO CIA FACTBOOK (40 metrics)
...
Comparison of CIA overlap metrics with primary sources (OWID, WHO, IMF):

### Use PRIMARY Source (CIA adds little value)

| Metric | CIA Coverage | Primary Source | Primary Coverage | Recommendation |
|--------|--------------|----------------|------------------|----------------|
...
### CIA Adds Historical Value

| Metric | CIA Coverage | Notes |
|--------|--------------|-------|
...
### CIA Adds Health Historical Value

| Metric | CIA Coverage | WHO Coverage | Notes |
|--------|--------------|--------------|-------|
| life_expectancy | 437 obs (2003-2019) | 185 obs (2021 only) | CIA has historical data |
...
| hiv_deaths | 659 obs (1999-2019) | Not in WHO | Unique to CIA |
| hiv_living | 799 obs (1999-2019) | Not in WHO | Unique to CIA |
...
- CO2 units differ: CIA in tonnes, OWID in million tonnes
...
|--------|------------|-------|-------|
| CIA Overlap | 1990-2020 | 31 | Best recent decade coverage |
...
**Key Insight**: For GDP, trade, inflation, and health metrics, CIA is often the ONLY source with 1990s data.

### Recommendation

1. **Keep cia_overlap excluded from catalog** - prevents duplicate/conflicting data in search
...
4. **Use for HIV metrics** - CIA has better historical HIV data (1999-2019) than current WHO
...
The CIA Factbook changed HTML structure significantly across editions. This section documents how to extract data from each era.
...
# Returns: [(cia_code, rank, country_name, value_str, year_str), ...]
```
...
# Convert country code to uppercase: cia_code.upper()
```
...
Full mappings stored in `cia_field_mappings.json`.
...
Based on scanning all editions 2010-2020. Detailed counts per metric in `cia_factbook_metrics.csv`.
...
A detailed CSV (`cia_factbook_metrics.csv`) tracks metric availability across all editions:
